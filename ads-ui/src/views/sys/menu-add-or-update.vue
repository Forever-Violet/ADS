          ...
          <template v-slot:reference>
            <el-input v-model="dataForm.parentName" :readonly="true" placeholder="上级菜单">
              <template v-slot:suffix>
                <el-icon v-if="dataForm.pid !== '0'" @click.stop="schoolListTreeSetDefaultHandle()" class="el-input__icon"><circle-close /></el-icon
              ></template>
            </el-input>
          </template>
...
const getInfo = (id: number) => {
  baseService.get(`/sys/menu/${id}`).then((res) => {
    ...
    if (dataForm.pid === "0") {
      return schoolListTreeSetDefaultHandle();
    }
...
// 上级菜单树, 设置默认值
const schoolListTreeSetDefaultHandle = () => {
  dataForm.pid = "0";
  dataForm.parentName = "一级菜单";
};
