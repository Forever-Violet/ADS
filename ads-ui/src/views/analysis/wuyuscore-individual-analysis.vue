<template>
  <el-dialog v-model="visible" title="" :close-on-click-modal="false" :close-on-press-escape="false" @opened="initChart" :key="dialogKey">
    ...
    <div>
      <canvas ref="chartContainer" width="100%" height="600"></canvas>
    </div>
    <el-form :model="dataForm" v-if="dataForm.id != ''" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="0px">
      <div id="pdfDom">
        <div v-if="loading" style="font-size: 16px; text-indent: 2em">
          {{ dataForm.response }}
        </div>
        <el-input type="textarea" v-if="!loading" v-model="dataForm.response" placeholder="请输入内容" :rows="4" style="font-size: 16px; text-indent: 2em" maxlength="1000" class="borderless-textarea"> </el-input>
      </div>
